        ...
        if result_task.result:
            if result_task.storage_link:
                await callback.message.answer(
                    external_storage_ms[language_code].format(
                        title=result_task.video.title,
...
async def check_privilege_and_load(
...
async def decrease_and_validate_limits(callback: CallbackQuery, language_code: str, worker_name: str):
    ...
    if not await validate_limit(callback.from_user.id, parameter):
        await callback.bot.send_message(chat_id=callback.from_user.id, text=get_limit_reply(parameter, language_code))
        return False
...
def check_content_type(message):
    ...
    logger.warning(
        "{username}:{user_id}:invalid file type:{type}",
